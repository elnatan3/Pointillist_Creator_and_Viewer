...
#    - would like to add lru_caching decorator to image loading
#        (so students can animate better by calling img.load(filename)
#          to change images being used, but Image is mutable due to
#          setPixel, so would need to copy-on-write when setPixel is called
#          for the first time?)
